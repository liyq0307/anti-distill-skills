...
This is the Claude Code discovery entrypoint for the canonical implementation in:
这是 Claude Code 的发现入口，真实实现位于：
...
If this project is the current Claude Code workspace root, the `.claude/skills/anti-distill` folder makes the skill auto-discoverable.
如果当前 Claude Code 工作区根目录就是这个仓库，那么 `.claude/skills/anti-distill` 会让技能自动被发现。
